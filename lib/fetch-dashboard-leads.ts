...
import { hydrateAssigneeNames } from "@/lib/hydrate-assignee-names";
import {
...
export type FetchDashboardLeadsOptions = {
  ...
  /** 관리자 전체 조회 시 처리 담당자 이름 users 조인 */
  withAssigneeJoin?: boolean;
};

/** Supabase/PostgREST 컬럼·스키마 오류 */
const COLUMN_MISSING =
  /pdf_url|docs_status|has_weim|other_docs|referrer|assigned_to|disease_category|fee_amount|current_owner_role|assigned_user_id|assignment_memo|is_read|callback_date|last_updated_at|master_agent_id|assignee|embedded|relationship|column|does not exist|schema cache|could not find/i;

type RawLeadRow = LeadDetail & {
  assignee?: { name: string } | { name: string }[] | null;
  users?: { name: string } | { name: string }[] | null;
};

function normalizeLeadRows(rows: unknown[]): LeadDetail[] {
  return (rows as RawLeadRow[]).map((row) => {
    const { assignee, users: usersEmbed, ...rest } = row;
    const embed = Array.isArray(assignee) ? assignee[0] : assignee;
    const usersName = Array.isArray(usersEmbed) ? usersEmbed[0]?.name : usersEmbed?.name;
    const joinedName = embed?.name ?? usersName ?? null;
    return {
      ...rest,
      assigned_user_name: rest.assigned_user_name ?? joinedName,
    };
  });
}
...
function buildSelectVariants(options: FetchDashboardLeadsOptions): string[] {
  ...
  if (options.admin) {
    if (options.withAssigneeJoin) {
      variants.push(...LEADS_LIST_SELECT_FALLBACKS);
    } else {
      variants.push(
        ...LEADS_LIST_SELECT_FALLBACKS.filter((s) => !s.includes("assignee:")),
      );
    }
  }
...
export async function fetchDashboardLeads(
  ...
  for (const select of selects) {
    ...
    if (!error) {
      const normalized = normalizeLeadRows(data ?? []);
      const hydrated = await hydrateAssigneeNames(normalized);
