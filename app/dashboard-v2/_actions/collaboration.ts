...
function excludeReminderNotifications(rows: V2Notification[]): V2Notification[] {
  return rows.filter((n) => n.kind !== "reminder");
}
...
export async function fetchV2Notifications(): Promise<{
  ...
  const notifications = excludeReminderNotifications(
    (data ?? []).map((row) => ({
      id: row.id as string,
